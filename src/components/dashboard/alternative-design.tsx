...
import { motion, AnimatePresence } from "framer-motion"
import { Plus, Search, ThumbsUp, ThumbsDown, Music, Send } from "lucide-react"
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { Avatar, AvatarFallback } from "@/components/ui/avatar"
        ...
        <div className="flex flex-col lg:flex-row gap-6 p-6">
          ...
          <div className="lg:w-1/3 flex flex-col justify-between">
            <div>
              ...
              <p className="mt-4 text-white/80">
                Currently playing in <strong>{roomInfo.name}</strong>. Add songs to the queue or vote for your favorites!
              </p>
            </div>
            <div className="mt-4 flex flex-col gap-3">
              <div className="flex gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <AnimatePresence initial={false}>
              {queueItems.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-medium line-clamp-2 text-white">{item.title}</h3>
                    <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                      <span>Added by {item.addedBy}</span>
                      <span>•</span>
                      <span>{item.votes} votes</span>
                  ...
                  <div className="flex flex-col gap-2">
                    <Button
                      ...
                      className={cn(
                        ...
                        item.userVote === "up" && "text-green-500 bg-green-500/10"
                      )}
                      onClick={() => handleVote(item.id, item.userVote === "up" ? null : "up")}
                      aria-label="Vote up"
                    >
                    ...
                    <Button
                      ...
                      className={cn(
                        "h-9 w-9 rounded-full",
                        item.userVote === "down" && "text-red-500 bg-red-500/10"
                      )}
                      onClick={() => handleVote(item.id, item.userVote === "down" ? null : "down")}
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <AnimatePresence initial={false}>
              {chatMessages.map((message, index) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-sm text-white">{message.user}</span>
                      <span className="text-xs text-muted-foreground">{message.time}</span>
                    </div>
                    <p className="text-sm mt-1 bg-gradient-to-r from-background to-background/50 p-2 rounded-lg border border-border">
                      {message.message}
            ...
            {chatMessages.length === 0 && (
              <p className="text-center text-muted-foreground py-10">No chat messages yet</p>
            )}
          </div>
...
// Helper function to fix userVote values and sort queue items before passing as prop
export function fixAndSortQueueItems(queueItems: AlternativeDesignProps["queueItems"]) {
  return [...queueItems]
    .map((item) => ({
      ...item,
      userVote: item.userVote === "up" || item.userVote === "down" ? item.userVote : null,
    }))
